...
class DecoderBlock(nn.Module):
    def __init__(self,channel_in,channel_out):
        ...
        self.conv = nn.ConvTranspose2d(channel_in, channel_out, 5, 2, 2,1, bias=False)
...
class Encoder(nn.Module):
    ...
    def __init__(self,channel_in=3,z_size=2):
        ...
        self.l_mu = nn.Sequential(nn.Linear(in_features=8*8*self.size,out_features=z_size),
                                  nn.BatchNorm1d(num_features=z_size))
        self.l_var = nn.Sequential(nn.Linear(in_features=8*8*self.size,out_features=z_size),
                                  nn.BatchNorm1d(num_features=z_size))
...
class Decoder(nn.Module):
    ...
    def __init__(self,z_size,size):
        ...
        #B*z_size*1*1
        self.fc = nn.Sequential(nn.Linear(in_features=z_size,out_features=8*8*size),
        ...
        for i in range(3):
            if i==2:
                layers_list.append(DecoderBlock(channel_in=self.size,channel_out=32))
                self.size = 32
            else:
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(comment="_CELEB")
    ...
    # OPTIM-LOSS
    optimizer = Adam(params=net.parameters(), lr=0.0075)

    a_kl = 1
    a_nle = 0.45
    lr_sc = StepLR(optimizer,step_size=1,gamma=0.95)
    ...
    for i in range(num_epochs):
        ...
        for j, (data_batch, labels_batch) in enumerate(dataloader):
            ...
            # propago TUTTO
            (a_kl*loss_kl_value+a_nle*loss_nle_value).backward()
            optimizer.step()
